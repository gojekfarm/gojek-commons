...
/**
 * A subtype resolver which discovers subtypes via {@code META-INF/services/com.gojek.util.serializer.Discoverable}.
 */
public class DiscoverableSubtypeResolver extends StdSubtypeResolver {
	...
	protected List<Class<?>> discoverServices(Class<?> klass) {
		...
		try {
			// use classloader that loaded this class to find the service
			// descriptors on the classpath
			// better than ClassLoader.getSystemResources() which may not be the
			// same classloader if ths app
